...
export default function MovieGrid({ movies, emptyIcon="🎬", emptyTitle="Nothing here yet", emptyText="Search above to add movies" }) {
  ...
  const handleCardClick = (movie) => {
    const targetId = movie.tmdbId || movie.id || movie._id;
    if (movie.type === "tv") {
  ...
  return (
    <div className="movies-grid">
      {movies.map((movie, i) => {
        ...
        return (
          <div key={movie._id} className="movie-card" style={{ animationDelay:`${i*0.04}s` }} onClick={() => handleCardClick(movie)}>
            <div className="card-poster-wrap">
              ...
              <div className="card-overlay">
                <button className="btn btn-ghost btn-xs" onClick={e => { e.stopPropagation(); handleCardClick(movie); }} style={{ fontSize:"0.72rem" }}>✏️ Edit</button>
              </div>
