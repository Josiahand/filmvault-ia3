...
export default function StatsPage() {
  ...
  useEffect(() => {
    api.get("/movies/stats").then(({ data }) => {
      setStats(data.stats);
      setLoading(false);
    }).catch(() => setLoading(false));
  }, [movies]); // re-fetch when movies change
  ...
  const localAvg = rated.length ? (rated.reduce((s, m) => s + m.rating, 0) / rated.length).toFixed(1) : "—";

  // Genre counts from local state
  const genreMap = {};
  watched.forEach((m) => { if (m.genre) genreMap[m.genre] = (genreMap[m.genre] || 0) + 1; });
  ...
  const topStats = [
    ...
    { value: genreList[0]?.[0] || "—", label: "Fav Genre", cls: "c-purple" },
  ];
        ...
        <div className="page-body">
          ...
          {genreList.length > 0 && (
            <div className="chart-card">
              <div className="chart-heading">Movies by Genre</div>
          ...
          {/* Rating distribution */}
          <div className="chart-card">
            <div className="chart-heading">Rating Distribution</div>
            <div className="bar-chart">
              {ratingDist.map(({ star, count }) => (
                <div key={star} className="bar-row">
                  <span className="bar-lbl" style={{ color: "var(--accent)" }}>{star} star{star !== 1 ? "s" : ""}</span>
                  <div className="bar-track">
                    <div
